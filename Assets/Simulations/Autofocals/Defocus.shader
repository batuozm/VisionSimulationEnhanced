Shader "Hidden/Defocus"
{
	...
	sampler2D _MainTex, _defocusTexture, _blurredTex;
	...
	SubShader
	{
		...
		Pass // 2 convolution pass
		{
			...
			{
				return saturate((coc - radius + 1) / 2);
			}
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				for (int k = 0; k < kernelSampleCount; k++)
				{
					...
					half coc_sample = tex2D(_defocusTexture, i.uv + o).r;
				...
				return half4(color, bgfg_interp);
			}
			ENDCG
		}
		...
		Pass // 4 combine pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half dofStrength = smoothstep(0.1, 1.0, abs(coc));
				half3 color = lerp(
